...
def w_dot(Uv, Ud, w, K, i, J):
    return ((-1*Uv*w - Ud*np.sign(w) + K*i)/J)
...
def plot(lambda0, w0, Wd0, Ud0, Uv0, tf, J, K):
    ...
    # Create sliders
    s_lambda = Slider(ax=ax_lambda, label='Lambda ', valmin=-100, valmax=500,
                  valinit=lambda0, valfmt='%i', facecolor='#cc7000')
    s_Wd = Slider(ax=ax_Wd, label='Wd', valmin=-200, valmax=200,
                 valinit=Wd0, valfmt='%i rad/s', facecolor='#cc7000')
    s_Ud = Slider(ax=ax_Ud, label='Ud', valmin=-1, valmax=1,
                 valinit=Ud0, valfmt='%i', facecolor='#cc7000')
    s_Uv = Slider(ax=ax_Uv, label='Uv', valmin=-1, valmax=1,
                 valinit=Uv0, valfmt='%i', facecolor='#cc7000')
